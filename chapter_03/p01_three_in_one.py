...
class MultiStack:
    ...
    def __init__(self,stack_size,number_of_stacks):
        ...
        self.capacity = self.size * self.n

        self.array = [0 for _ in range(self.capacity)]
        self.sizes = [0 for _ in range(self.n)]



    def pop(self,stack_num):
        val = self.peek(stack_num)
    ...
    def push(self,stack_num,val):
        self._is_valid_stack_num(stack_num)
        if self.is_full(stack_num):
            raise StackFullError(f"Push failed: stack #{stack_num} is full")
        self.sizes[stack_num] += 1
        top_index = self._index_top(stack_num)
        self.array[top_index] = val
    ...
    def is_full(self,stack_num):
        self._is_valid_stack_num(stack_num)
        return self.sizes[stack_num] == self.size

    def _index_top(self,stack_num):
        self._is_valid_stack_num(stack_num)
        offset = stack_num*self.size
        return offset + self.sizes[stack_num] - 1

    def _is_valid_stack_num(self,stack_num):
        if stack_num >= self.n:
